...
- Product grids collapse 4 → 2 → 1. **Service tiles stay two across on a phone**, with the lead tile spanning both columns: five services then read as one feature plus a 2x2 grid rather than a ragged row. At 166px every non-wide tile goes square, drops its tagline and drops the arrow puck, because a 4:3 frame left 124px for three things that each needed the space. This applies to the `large` tiles on the services page too, not just the homepage's: they were missed once and shipped at 166x125 with a `text-xl` title inside.
